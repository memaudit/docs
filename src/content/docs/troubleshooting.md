...
itself — safe to run anywhere, anytime.
...
- **`kernel <version>`** — always shows `OK` with the detected kernel
  version; this line never fails, it's informational (later checks
  reference the kernel version's implications).
...
- **`DAMON sysfs`**, **`DAMON paddr`**, **`DAMON tried_regions
  ...
  other collector still works normally.
- **`DAMON tried_regions (>=6.2)`** specifically has a different detail
  ...
  string, which this static version check can't detect — but
  `memauditd` probes for the real capability at startup regardless of
  what this line says, so a `FAIL` here doesn't guarantee the feature
  is actually missing. Don't treat this line as final; check the
  agent's own startup log on a host where this matters.
- **`PSI`** — `FAIL: PSI unavailable — stranded estimate not computed`
  ...
  signal won't be computed; every other collector still works.
...
- **An old kernel reporting combined `workingset_refault` /
  ...
  memaudit stores (pre-5.8 kernels) — those two unsplit keys are
  ignored (left zero), not treated as an error.
